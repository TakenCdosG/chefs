...
*/
      ...
      if (($vtprd_rule->cart_or_catalog_select == 'choose') || ($vtprd_rule->cart_or_catalog_select <= ' ')) {
          $vtprd_rule->rule_error_message[] = array( 
                'insert_error_before_selector' => '.top-box',  
                'error_msg'  => __('Cart or Catalog choice not yet made', 'vtprd') );   //mwn20140414       
      ...
      if (($vtprd_rule->minimum_purchase_select == 'choose') || ($vtprd_rule->minimum_purchase_select <= ' ')) {
          $vtprd_rule->rule_error_message[] = array( 
                'insert_error_before_selector' => '.top-box',  
                'error_msg'  => __('Deal Action choice not yet made', 'vtprd') );   //mwn20140414       
        ...
        case 'groups':
             ...
             if ((!$vtprd_rule->prodcat_in_checked) && (!$vtprd_rule->rulecat_in_checked) && (!$vtprd_rule->role_in_checked))  {
                $vtprd_rule->rule_error_message[] = array( 
                              ...
                              'error_msg'  => __('In Cart Search Criteria Selection Metabox, "Category / Logged-in Role / Custom Category" was chosen, but no Categories or Roles checked', 'vtprd') );
             ...
             switch( $vtprd_rule->role_and_or_in ) {
                case 'and':
                  //  $vtprd_rule->role_and_or_in[0]['user_input'] = $selected;
                    if ((!$vtprd_rule->prodcat_in_checked) && (!$vtprd_rule->rulecat_in_checked) && (!$vtprd_rule->role_in_checked))  {
                        $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#buy_group_box_0',  
                              'error_msg'  => __('In "Buy" Group Selection, "Category / Logged-in Role / Custom Category" and "And" was chosen, but no Categories or Roles are checked', 'vtprd') );
                        $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
                        $vtprd_rule->rule_error_red_fields[] = '#prodcat-in-label';
                        $vtprd_rule->rule_error_red_fields[] = '#rulecat-in-label';
                        $vtprd_rule->rule_error_red_fields[] = '#role-in-label';                     
                    ...
                    if ((!$vtprd_rule->prodcat_in_checked) && (!$vtprd_rule->rulecat_in_checked)) {
                        $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#buy_group_box_0',  
                              'error_msg'  => __('In "Buy" Group Selection, "Category / Logged-in Role / Custom Category" and "And" was chosen, but no Categories are checked. <br><br><br><br><em>If Roles alone are desired, please select "OR".</em>', 'vtprd') );
                        $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
                        $vtprd_rule->rule_error_red_fields[] = '#prodcat-in-label';
                        $vtprd_rule->rule_error_red_fields[] = '#rulecat-in-label';                   
                    } else
                    if (!$vtprd_rule->role_in_checked)  {
                        $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#buy_group_box_0',  
                              'error_msg'  => __('In "Buy" Group Selection, "Category / Logged-in Role / Custom Category" and "And" was chosen, but no Roles are checked.  <br><br><br><br><em>If Categories alone are desired, please select "OR".</em>', 'vtprd') );
                        $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
                        $vtprd_rule->rule_error_red_fields[] = '#role-in-label';                    
                    }
                  break;
                case 'or':
                  ...
                  break;
                default:  
                      $vtprd_rule->role_and_or_in = 'or'; //v1.0.5.5  //initialize so it's always there.  overwritten by logic as needed.
                  break;  
              }
          break;
        case 'vargroup':
         //   $vtprd_rule->inPop[2]['user_input'] = $selected; 
            ...
            if ( ($vtprd_rule->inPop_varProdID == '') || ($vtprd_rule->inPop_varProdID == ' ') ){
              $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#vtprd-buy-action-groups',         
                              'error_msg'  => __('In "Buy" Group Selection, "*Single Product with variations* was chosen, but Product ID was not supplied.', 'vtprd') );
              $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
              $vtprd_rule->rule_error_red_fields[] = '#varProdID-in-label';
            }  
            $vtprd_rule->inPop_varProdID = preg_replace('/[^0-9.]+/', '', $vtprd_rule->inPop_varProdID); //remove leading/trailing spaces, percent sign, dollar sign
            if ( is_numeric($vtprd_rule->inPop_varProdID) === false ) {
               $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#inPop-varProdID-cntl',  
                              'error_msg'  =>  __('In "Buy" Group Selection, "*Single Product with variations* was chosen, but Product ID was not numeric.', 'vtprd') );
               $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
               $vtprd_rule->rule_error_red_fields[] = '#varProdID-in-label';
            } else { 
                ...
                if ( !$test_post ) {
                   $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#inPop-varProdID-cntl',
                              'error_msg'  => __('In "Buy" Group Selection, "*Single Product with variations* was chosen, but Product ID was not found.', 'vtprd') );
                   $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
                   $vtprd_rule->rule_error_red_fields[] = '#varProdID-in-label';
                }  else {
                   ...
                   if ($product_has_variations == 'yes') {
                     if(!empty($_REQUEST['tax-input-var-in'])) { 
                      ...
                      //  $vtprd_rule->var_in_checked_min_requirement = $_REQUEST["var-list-in-min-requirement"];
                        ...
                        $checkbox_count = $_REQUEST['checkbox_count-var-in'];  //checkbox_count-var-in hidden field from ajax... 
                        switch( true ) {  
...
//mwnt                     echo 'var_in_checked <pre>'.print_r($vtprd_rule->var_in_checked, true).'</pre>' ;
                     ...
                     //if (!$vtprd_rule->var_in_checked) {
                       $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#inPop-varProdID-cntl', 
                              'error_msg'  => __('In "Buy" Group Selection, "Single Product with variations" was chosen, but no Variations checked', 'vtprd') );
                       $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
                       $vtprd_rule->rule_error_red_fields[] = '#varProdID-in-label';
                     }
                  } else {
                     $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#inPop-varProdID-cntl', 
                              'error_msg'  =>  __('In "Buy" Group Selection, "Single Product with variations" was chosen, but no Product has no Variations.  Please use "Single Product Only" option.', 'vtprd') );
                     $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
                     $vtprd_rule->rule_error_red_fields[] = '#varProdID-in-label';
                  }
                }
        ...
        case 'single':                                        
            ...
            if ($vtprd_rule->inPop_singleProdID == ' '){
              $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#buy_group_box_0',  
                              'error_msg'  => __('In "Buy" Group Selection, "*Single Product Only* was chosen, but Product ID was not supplied.', 'vtprd') );
              $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label' ;
              $vtprd_rule->rule_error_red_fields[] = '#singleProdID-in-label';
            }  
            $vtprd_rule->inPop_singleProdID =  preg_replace('/[^0-9.]+/', '', $vtprd_rule->inPop_singleProdID); //remove leading/trailing spaces, percent sign, dollar sign
            if ( is_numeric($vtprd_rule->inPop_singleProdID) === false ) {
               $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#buy_group_box_0',  
                              'error_msg'  => __('In "Buy" Group Selection, "*Single Product Only* was chosen, but Product ID was not numeric.', 'vtprd') );
              $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label' ;
              $vtprd_rule->rule_error_red_fields[] = '#singleProdID-in-label';           
            } else { 
                ...
                if ( !$test_post ) {
                   $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#buy_group_box_0',  
                              'error_msg'  => __('In "Buy" Group Selection, "*Single Product Only* was chosen, but Product ID was not found.', 'vtprd') );
                   $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
                   $vtprd_rule->rule_error_red_fields[] = '#singleProdID-in-label';                
                }  else {
                   ...
                   if ($product_has_variations == 'yes') {
                        $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#buy_group_box_0',  
                              'error_msg'  => __('In "Buy" Group Selection, "Single Product Only" was chosen, but ** Product has Variations **.  Please use "Single Product with variations" option.', 'vtprd') );
                        $vtprd_rule->rule_error_red_fields[] = '#inPopChoiceIn_label';
                        $vtprd_rule->rule_error_red_fields[] = '#singleProdID-in-label';                   
                   } 
                }
        ...
        case 'groups':
             ...
             switch( $vtprd_rule->role_and_or_out ) {
                case 'and':
                  //  $vtprd_rule->role_and_or_out[0]['user_input'] = $selected;
                    if ((!$vtprd_rule->prodcat_out_checked) && (!$vtprd_rule->rulecat_out_checked) && (!$vtprd_rule->role_out_checked))  {
                        $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#action_group_box_0',  
                              'error_msg'  => __('In "Action" Group Selection, "Category / Logged-in Role / Custom Category" and "And" was chosen, but no Categories or Roles are checked', 'vtprd') );
                        $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
                        $vtprd_rule->rule_error_red_fields[] = '#prodcat-out-label';
                        $vtprd_rule->rule_error_red_fields[] = '#rulecat-out-label';
                        $vtprd_rule->rule_error_red_fields[] = '#role-out-label';                     
                    ...
                    if ((!$vtprd_rule->prodcat_out_checked) && (!$vtprd_rule->rulecat_out_checked)) {
                        $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#action_group_box_0',  
                              'error_msg'  => __('In "Action" Group Selection, "Category / Logged-in Role / Custom Category" and "And" was chosen, but no Categories are checked. <br><br><br><br><em>If Roles alone are desired, please select "OR".</em>', 'vtprd') );
                        $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
                        $vtprd_rule->rule_error_red_fields[] = '#prodcat-out-label';
                        $vtprd_rule->rule_error_red_fields[] = '#rulecat-out-label';                   
                    } else
                    if (!$vtprd_rule->role_out_checked)  {
                        $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#action_group_box_0',  
                              'error_msg'  => __('In "Action" Group Selection, "Category / Logged-in Role / Custom Category" and "And" was chosen, but no Roles are checked.  <br><br><br><br><em>If Categories alone are desired, please select "OR".</em>', 'vtprd') );
                        $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
                        $vtprd_rule->rule_error_red_fields[] = '#role-out-label';                    
                    }
                  break;
                case 'or':
                  ...
                  break;
                default:  
                    $vtprd_rule->role_and_or_out = 'or'; //v1.0.5.5 //initialize so it's always there.  overwritten by logic as needed.
                  break;  
              }
          break;
        case 'vargroup':
            ...
            if ( ($vtprd_rule->actionPop_varProdID == '') || ($vtprd_rule->actionPop_varProdID == ' ') ){
              $vtprd_rule->rule_error_message[] = array( 
                              ...
                              'error_msg'  => __('In "Action" Group Selection, "*Single Product with variations* was chosen, but Product ID was not supplied.', 'vtprd') ); 
              $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
              $vtprd_rule->rule_error_red_fields[] = '#varProdID-out-label';
            }  
            $vtprd_rule->actionPop_varProdID =  preg_replace('/[^0-9.]+/', '', $vtprd_rule->actionPop_varProdID); //remove leading/trailing spaces, percent sign, dollar sign
            if ( is_numeric($vtprd_rule->actionPop_varProdID) === false ) {
               $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#actionPop-varProdID-cntl',  
                              'error_msg'  =>  __('In "Action" Group Selection, "*Single Product with variations* was chosen, but Product ID was not numeric.', 'vtprd') ); 
               $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
               $vtprd_rule->rule_error_red_fields[] = '#varProdID-out-label';
            } else { 
                ...
                if ( !$test_post ) {
                   $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#actionPop-varProdID-cntl',  
                              'error_msg'  => __('In "Action" Group Selection, "*Single Product with variations* was chosen, but Product ID was not found.', 'vtprd') );
                   $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
                   $vtprd_rule->rule_error_red_fields[] = '#varProdID-out-label';
                }  else {
                   ...
                   if ($product_has_variations == 'yes') {
                    ...
                    //    $vtprd_rule->var_out_checked_min_requirement = $_REQUEST["var-list-out-min-requirement"];
                     ...
                     //if (!$vtprd_rule->var_out_checked) {
                       $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#actionPop-varProdID-cntl',  
                              'error_msg'  => __('In "Action" Group Selection, "Single Product with variations" was chosen, but no Variations checked', 'vtprd') );
                       $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
                       $vtprd_rule->rule_error_red_fields[] = '#varProdID-out-label';
                     }
                  } else {
                     $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#actionPop-varProdID-cntl',  
                              'error_msg'  =>  __('In "Action" Group Selection, "Single Product with variations" was chosen, but no Product has no Variations.  Please use "Single Product Only" option.', 'vtprd') );
                     $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
                     $vtprd_rule->rule_error_red_fields[] = '#varProdID-out-label';
                  }
                }
        ...
        case 'single':
         //   $vtprd_rule->actionPop[3]['user_input'] = $selected;                                        
            ...
            if ($vtprd_rule->actionPop_singleProdID == ' '){
              $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#action_group_box_0',  
                              'error_msg'  => __('In "Action" Group Selection, "*Single Product Only* was chosen, but Product ID was not supplied.', 'vtprd') );
              $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label' ;
              $vtprd_rule->rule_error_red_fields[] = '#singleProdID-out-label';
            }  
            $vtprd_rule->actionPop_singleProdID = preg_replace('/[^0-9.]+/', '', $vtprd_rule->actionPop_singleProdID); //remove leading/trailing spaces, percent sign, dollar sign
            if ( is_numeric($vtprd_rule->actionPop_singleProdID) === false ) {
               $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#action_group_box_0',  
                              'error_msg'  => __('In "Action" Group Selection, "*Single Product Only* was chosen, but Product ID was not numeric.', 'vtprd') );
              $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label' ;
              $vtprd_rule->rule_error_red_fields[] = '#singleProdID-out-label';           
            } else { 
                ...
                if ( !$test_post ) {
                   $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#action_group_box_0',  
                              'error_msg'  => __('In "Action" Group Selection, "*Single Product Only* was chosen, but Product ID was not found.', 'vtprd') );
                   $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
                   $vtprd_rule->rule_error_red_fields[] = '#singleProdID-out-label';                
                }  else {
                   ...
                   if ($product_has_variations == 'yes') {
                        $vtprd_rule->rule_error_message[] = array( 
                              'insert_error_before_selector' => '#action_group_box_0',  
                              'error_msg'  => __('In "Action" Group Selection, "Single Product Only" was chosen, but ** Product has Variations **.  Please use "Single Product with variations" option.', 'vtprd') );
                        $vtprd_rule->rule_error_red_fields[] = '#actionPopChoiceOut_label';
                        $vtprd_rule->rule_error_red_fields[] = '#singleProdID-out-label';                   
                   } 
                }
      ...
      for($d=0; $d < $sizeof_rule_deal_info; $d++) {                  
         if ($vtprd_rule->rule_deal_info[$d]['discount_auto_add_free_product'] == 'yes') {
            ...
            if ($vtprd_rule->actionPop  == 'sameAsInPop' ) {               
               if ( ($vtprd_rule->inPop      == 'vargroup') &&
                    (sizeof($vtprd_rule->var_in_checked) == 1 ) )  {                    
              ...
              $vtprd_rule->rule_error_message[] = array( 
                          'insert_error_before_selector' => '#buy_group_box_0',  
                          'error_msg'  => __('"Buy Group Filter" must be either a single product, or a single product variation, When "Discount Group Same as Buy Group" and "Automatically Add Free Product to Cart" is Selected.', 'vtprd') );
            ...
            if ( ($vtprd_rule->actionPop  == 'vargroup' ) &&
                 (sizeof($vtprd_rule->var_out_checked) == 1 ) )  {                 
            ...
            if (($vtprd_rule->inPop      == 'vargroup') &&
                ($vtprd_rule->actionPop  == 'vargroup' ) ) {                
               if (($vtprd_rule->inPop_varProdID == $vtprd_rule->actionPop_varProdID) &&
                   ($vtprd_rule->var_in_checked == $vtprd_rule->var_out_checked )) {                    
                  $vtprd_rule->auto_add_free_trigger_rule_type = 'same_product';
               } else {                
  ...
  */
  ...
  public function vtprd_show_pop() {  
    ...
    switch( $vtprd_rule->inPop ) {
      ...
      case 'groups':            
          if ( ($vtprd_rule->actionPop == 'sameAsInPop') ||              //in these cases, inpop/actionpop treated as 'sameAsInPop'
               ($vtprd_rule->actionPop == 'wholeStore') ||
               ($vtprd_rule->actionPop == 'cart') ) {
            if ($vtprd_rule->rule_template == 'D-simpleDiscount') {
              $output .= __(' The product must be in the listed ', 'vtprd');
            } else {
              $output .= __(' The cart contents must be in the listed ', 'vtprd');
            }
          } else {
            if ($vtprd_rule->rule_template == 'D-simpleDiscount') {
              $output .= __(' The product must be in the listed ', 'vtprd');
            } else {
              $output .= __(' The Buy Group must be in the listed ', 'vtprd');
            }           
          }
          switch( true ) {
            case ( ( sizeof($vtprd_rule->prodcat_in_checked) > 0 ) && ( sizeof($vtprd_rule->rulecat_in_checked) > 0 ) ):
                $output .=  'Product Categories or Pricing Deal Categories ';
                if ( sizeof($vtprd_rule->role_in_checked) > 0 ) {
                   if ($vtprd_rule->role_and_or_in  == 'and') {
                      $output .=  __(' AND in the listed Roles ', 'vtprd');
                   } else {
                      $output .=  __(' OR in the listed Roles ', 'vtprd');
                   }
                }
              break;
            case ( sizeof($vtprd_rule->prodcat_in_checked) > 0 ):
              ...
              break;
            case ( sizeof($vtprd_rule->rulecat_in_checked) > 0 ):
                $output .=  'Pricing Deal Categories ';
                if ( sizeof($vtprd_rule->role_in_checked) > 0 ) {
                   if ($vtprd_rule->role_and_or_in  == 'and') {
                      $output .=  __(' AND in the listed Roles ', 'vtprd');
                   } else {
              ...
              break; 
            case ( sizeof($vtprd_rule->role_in_checked) > 0 ):
                $output .=  __('Roles. ', 'vtprd');
              break;                                       
          }        
        break;
      case 'vargroup':
          if ( ($vtprd_rule->actionPop == 'sameAsInPop') ||              //in these cases, inpop/actionpop treated as 'sameAsInPop'
               ($vtprd_rule->actionPop == 'wholeStore') ||
               ($vtprd_rule->actionPop == 'cart') ) {
            $output .= __(' The cart contents must be in the listed Product Variation Group ', 'vtprd');
          } else {
            $output .= __(' The Buy Group must be in the listed Product Variation Group ', 'vtprd');
          }        
        break;
      case 'single':              
          if ( ($vtprd_rule->actionPop == 'sameAsInPop') ||              //in these cases, inpop/actionpop treated as 'sameAsInPop'
               ($vtprd_rule->actionPop == 'wholeStore') ||
               ($vtprd_rule->actionPop == 'cart') ) {
            $output .= __('. The cart contents include the listed Product ', 'vtprd');
          } else {
            $output .= __('. The Buy Group must include the listed Product ', 'vtprd');
          }        
        break;         
    }
   ...
   switch( $vtprd_rule->actionPop ) { 
      ...
      case 'groups':
          $output .= '<br>&nbsp;&nbsp;&nbsp; -';
          $output .= __('The discount "Action" Group must be in the listed ', 'vtprd');
          switch( true ) {
            case ( ( sizeof($vtprd_rule->prodcat_out_checked) > 0 ) && ( sizeof($vtprd_rule->rulecat_out_checked) > 0 ) ):
                $output .=  'Product Categories or Pricing Deal Categories ';
                if ( sizeof($vtprd_rule->role_in_checked) > 0 ) {
                   if ($vtprd_rule->role_and_or_out  == 'and') {
                      $output .=  __(' AND in the listed Roles ', 'vtprd');
                   } else {
                      $output .=  __(' OR in the listed Roles ', 'vtprd');
                   }
              ...
              break;
            case ( sizeof($vtprd_rule->prodcat_out_checked) > 0 ):
                $output .=  'Product Categories ';
                if ( sizeof($vtprd_rule->role_in_checked) > 0 ) {
                   if ($vtprd_rule->role_and_or_out  == 'and') {
                      $output .=  __(' AND in the listed Roles ', 'vtprd');
              ...
              break;
            case ( sizeof($vtprd_rule->rulecat_out_checked) > 0 ):
                $output .=  'Pricing Deal Categories ';
                if ( sizeof($vtprd_rule->role_in_checked) > 0 ) {
                   if ($vtprd_rule->role_and_or_out  == 'and') {
                      $output .=  __(' AND in the listed Roles ', 'vtprd');
      ...
      case 'vargroup':
          $output .= '<br>&nbsp;&nbsp;&nbsp; -';
          $output .= __(' The Get Group must be in the listed Product Variation Group ', 'vtprd');
        break;
      case 'single':
          $output .= '<br>&nbsp;&nbsp;&nbsp; -';
          $output .= __(' The Get Group must be in the listed Product ', 'vtprd');          
        break;
    }      
/*   
     //**********************************************************
 ...
 //default to 'OR', as the default value goes away and may be needed if the user switches back to 'groups'...
                    ...
                    if ( ($fieldName = 'discount_auto_add_free_product') &&
                        ...
                        if ( !in_array($field_value_temp, $fieldAttributes['cross_field_edits'][$c]['cross_allowed_values']) ) { 
                          ...
                          if ($cross_field_name == 'popChoiceOut') {
                            ...
                            switch( $field_value_temp ) {
                                case 'vargroup':      
                                      //don't have this yet, put to temp
                                      $var_out_checked_temp = $_REQUEST['tax-input-var-out'];
                                      if (sizeof($var_out_checked_temp) > 1) {
                                        $field_has_an_error = 'yes';
                                        $display_this_msg = __('Only a single variation selection (or single product) is allowed, When "Automatically Add Free Product to Cart" is Selected.', 'vtprd');          
                                        $vtprd_rule->rule_error_red_fields[] = '#action_group_title_anchor' ;
                                        $insertBefore = $fieldAttributes['cross_field_edits'][$c]['cross_field_insertBefore'];
                                        $this->vtprd_add_cross_field_error_message($insertBefore, $k, $display_this_msg, $fieldName);        
                                      } /*
                                      MOVED to MAINLINE EDITS
                                      else {
                                        //No edit error, but process data for future use
                                        //get data to support discount_auto_add_free_product for single var product selected
                                        $product_ID = $vtprd_rule->var_out_checked[0];
                                        //this value is null up to here, where the get_var function returns an array
                                        $vtprd_rule->var_out_product_variations_parameter  = vtprd_get_var_out_product_variations_parameter($product_ID);
                                      }  */                   
                                  break;              
                                case 'single':   
                                    //all set, do nothing
  ...
  public function vtprd_maybe_clear_extraneous_data() { 
    ...
    switch( $vtprd_rule->inPop ) {
      ...
      case 'vargroup':
          ...
          $vtprd_rule->prodcat_in_checked = array();
          $vtprd_rule->rulecat_in_checked = array();
         // $vtprd_rule->role_in_checked = array(); //v1.1 now alive
          $vtprd_rule->role_and_or_in = null;       
        break;
      case 'single':
          //clear vargroup
  ...
  public function vtprd_maybe_pop_filter_agreement() { 
    ...
    if (($vtprd_rule->inPop      ==  'groups')  &&
       ...
       if (($vtprd_rule->prodcat_in_checked  ==  $vtprd_rule->prodcat_out_checked) &&
           ($vtprd_rule->rulecat_in_checked  ==  $vtprd_rule->rulecat_out_checked) ) {
              $vtprd_rule->set_actionPop_same_as_inPop = 'yes';
              return;
       }                
    } 
