...
function chef_login_form()
{
  global $options;
  $options = get_option('register_login'); ?>
  <form id="login" class="crl-ajax-auth" action="login" method="post">
    <h1><?php if ($options['login_title']) {
        echo $options['login_title'];
      } else {
        echo 'Login';
      } ?></h1>
    <hr/>
    ...
    <input class="<?php if ($options['button_class']) {
      echo $options['button_class'];
    } else {
      echo 'button';
    }; ?>" type="submit" value="<?php if ($options['button_login']) {
      echo $options['button_login'];
    } else {
      echo 'Login';
    }; ?>" name="login">
  ...
  <form id="register" class="crl-ajax-auth" action="register" method="post">
    <h1><?php if ($options['register_title']) {
        echo $options['register_title'];
      } else {
  ...
  <form id="forgot_password" class="crl-ajax-auth" action="forgot_password" method="post">
    <h1><?php if ($options['forgot_password_title']) {
        echo $options['forgot_password_title'];
      } else {
    ...
    <div for="user_login"><span class="label">Username or Email</span>
      <input id="user_login" type="text" class="required" name="user_login"
             placeholder="Insert your username or email here"></div>
    <input class="<?php if ($options['button_class']) {
      echo $options['button_class'];
    } else {
      echo 'button';
    }; ?>" type="submit" value="<?php if ($options['button_forgot_password']) {
      echo $options['button_forgot_password'];
    } else {
      echo 'Get Password';
...
add_action("wp_footer", "chef_login_form");
...
add_shortcode('chef_login', 'chef_login');
...
add_shortcode('chef_register', 'chef_register');

function chef_logout($atts, $content = null)
{
  if (is_user_logged_in()) {
...
add_shortcode('chef_logout', 'chef_logout');
