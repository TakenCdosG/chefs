<div class="wrap">
	...
	<h2><?php esc_html_e( 'Orders', 'event-tickets-plus' ); ?></h2>
			...
			<table class="eventtable ticket_list">
				<tr>
					<td width="33%" valign="top">
						<h4><?php esc_html_e( 'Event Summary', 'event-tickets-plus' ); ?></h4>
						<div class="tribe-event-meta tribe-event-meta-date">
							<strong><?php echo esc_html__( 'Date:', 'event-tickets-plus' ); ?></strong>
							<?php echo esc_html( tribe_get_start_date( $event, false ) ); ?>
						</div>
						<div class="tribe-event-meta tribe-event-meta-id">
							<strong><?php echo esc_html__( 'Event ID:', 'event-tickets-plus' ); ?></strong>
							<?php echo absint( $event_id ); ?>
						</div>
						<div class="tribe-event-meta tribe-event-meta-organizer">
							<strong><?php echo esc_html__( 'Organizer:', 'event-tickets-plus' ); ?></strong>
							...
							<?php echo esc_html( sprintf( _x( ' (ID: %s)', 'ID of community organizer', 'event-tickets-plus' ), absint( $event->post_author ) ) ); ?>
						</div>
						<?php do_action( 'tribe_events_community_orders_report_after_organizer', $event, $organizer ); ?>
					</td>
					<td width="33%" valign="top">
						<h4><?php esc_html_e( 'Ticket Sales', 'event-tickets-plus' ); ?></h4>
						<div class="tribe-event-meta tribe-event-meta-tickets-sold">
							<strong><?php echo esc_html__( 'Tickets sold:', 'event-tickets-plus' ); ?></strong>
							<?php echo absint( $total_sold ); ?>
							<?php if ( $total_pending > 0 ) : ?>
								<div id="sales_breakdown_wrapper" class="tribe-event-meta-note">
									<div>
										<?php esc_html_e( 'Completed:', 'event-tickets-plus' ); ?>
										<span id="total_issued"><?php echo esc_html( $total_completed ); ?></span>
									</div>
									<div>
										<?php esc_html_e( 'Awaiting review:', 'event-tickets-plus' ); ?>
					...
					<td width="33%" valign="top">
						...
						<div class="tribe-event-meta tribe-event-meta-total-revenue">
							<strong><?php esc_html_e( 'Total Revenue:', 'event-tickets-plus' ) ?></strong>
							<?php
							...
							if ( $event_fees ) {
								...
								<div class="tribe-event-meta-note">
									<?php echo esc_html__( '(Tickets + Site Fees)', 'event-tickets-plus' ); ?>
								</div>
								<?php
							}
							?>
